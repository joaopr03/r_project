...
filtered_data <- data[data$Country == "Italy" & data$TIME == "2019" & data$Age.Group %in% c("15-24", "25-54", "55-64") & data$Sex %in% c("Men", "Women") & data$IND == "EMP2", ]
...
ggplot(filtered_data, aes(x = AGE, y = Value, fill = SEX)) + 
  ...
  labs(title = "Labour force participation rate, by sex and age group in Italy (2019)",
       x = "Age Group",
